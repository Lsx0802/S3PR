...
a_file = open("RL_ckpt/configs.txt", "w")
for row in configs:
...
while i_episode<n_episodes:
    ...
    if done:
        ...
        if np.mean(rewards_window)>reward_max:
            ...
            torch.save(model.state_dict(), 'RL_ckpt/checkpoint.pth')
            a_file = open("RL_ckpt/best_ckpt.txt", "w")
            a_file.write(str([i_episode,reward_max])+'\r')
            a_file.close()
        
        if i_episode%save_every==0:
            torch.save(model.state_dict(), 'RL_ckpt/checkpoint_latest.pth')
        success_rate=len(np.where(np.array(done_his)==1)[0])
        
        if success_rate>best_success_rate:
            best_success_rate=success_rate
            if i_episode>100:
                torch.save(model.state_dict(), 'RL_ckpt/checkpoint_best_sr.pth')
                a_file = open("RL_ckpt/best_ckpt_sr.txt", "w")
